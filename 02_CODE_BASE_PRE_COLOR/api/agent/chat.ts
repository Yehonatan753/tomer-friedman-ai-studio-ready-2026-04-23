...
const SYSTEM_PROMPT = `
אתה סוכן התאמת מוצר באתר של תומר פרידמן, תזונאי קליני וספורט.
ענה בעברית, RTL-friendly, קצר, חד, בגוף ראשון בשם האתר אך לא תתחזה לתומר.
...
4. ליווי אישי מלא: עבודה צמודה עם תומר. ₪9,000 ל-3 חודשים או ₪16,500 ל-6 חודשים.
...
function fallbackAnswer(lastUserMessage: string) {
  ...
  if (text.includes('ביטוח') || text.includes('החזר')) {
    return 'ייתכן שיש החזרים דרך ביטוח פרטי או קופות, אבל זה תלוי בפוליסה שלך. הכי נכון להשאיר פרטים באזור הביטוח באתר ותומר יחזור אליך לבדיקה נקודתית.';
  }
...
async function callGemini(messages: { role: 'user' | 'assistant'; content: string }[]) {
  ...
  return {
    answer: answer || 'לא הצלחתי לייצר תשובה כרגע. אפשר להשאיר פרטים ותומר יחזור אליך בוואטסאפ.',
    configured: true,
  };
}
