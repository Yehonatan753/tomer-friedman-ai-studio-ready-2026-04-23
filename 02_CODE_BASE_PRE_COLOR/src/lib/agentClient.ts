...
function localAgentFallback(messages: AgentMessage[]): AgentResponse {
  ...
  if (lastUserMessage.includes('מחיר') || lastUserMessage.includes('כמה') || lastUserMessage.includes('עלות')) {
    return {
      ...
      configured: false,
      answer:
        'המחירים המרכזיים: TF Tracker עם חודש ראשון חינם בלבד ואז ₪49.90 חודשי או ₪39.90 במסלול שנתי. אחרי ייעוץ טלפוני מקבלים 3 חודשים חינם. תהליך אונליין מתחיל מ-₪4,500 ל-3 חודשים וליווי אישי מלא מ-₪9,000 ל-3 חודשים, ושניהם כוללים גישת Lifetime לאפליקציה.',
    };
  }
  ...
  if (lastUserMessage.includes('ביטוח') || lastUserMessage.includes('החזר')) {
    return {
      ok: true,
      configured: false,
      answer: 'יכול להיות שיש זכאות להחזר דרך ביטוח פרטי או קופה, אבל זה תלוי בפוליסה. השאר פרטים באזור הביטוח ואחזור אליך לבדיקה נקודתית.',
    };
  }
  ...
  if (lastUserMessage.includes('tracker') || lastUserMessage.includes('אפליק')) {
    return {
      ...
      configured: false,
      answer:
        'TF Tracker מתאים למי שרוצה להתחיל עצמאית וזול: אוכל, אימונים, מדידות ומעקב במקום אחד. חודש ראשון חינם בלבד, אחרי ייעוץ טלפוני 3 חודשים חינם, ובתהליך אונליין/פרונטלי יש גישת Lifetime כך שהנתונים ממשיכים איתך ולא מתחילים מאפס.',
    };
  }
